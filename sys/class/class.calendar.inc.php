...
class Calendar extends DB_connect {
    ...
    public function __construct($dbo = NULL, $useDate = NULL) {
        ...
        if (isset($month) && isset($year)) {
            $useDate = $year . '-' . $month . '-' . '01 00:00:00';
        }
         ...
         */
        $ts = mktime(0, 0, 0, $this->_m, 0, $this->_y);
    ...
    public function buildCalendar() {
        ...
        $nextMonth = $this->_m == 12 ? 1 : intval($this->_m) + 1;

        $nextYear = $this->_m == 12 ? intval($this->_y) + 1 : $this->_y;

        $preMonth = $this->_m == 1 ? 12 : intval($this->_m) - 1;

        $preYear = $this->_m == 1 ? intval($this->_y) - 1 : $this->_y;
        ...
        $previous_month = sprintf('%02d', $preMonth);
        
        /*
         * Add a header to the calendar markup
         */
        $html = '<div style="text-align:center">';
        $html .= "\n\t<a href=\"index.php?month=$previous_month&year=$preYear\" id=\"previousmonth\" class=\"prev-month\" title=\"mois précédent\"></a>";
        $html .= "\n\t<span id=\"month-$cal_id\">$cal_month</span>";
        $html .= "\n\t<a href=\"index.php?month=$next_month&year=$nextYear\" id=\"nextmonth\" class=\"next-month\" title=\"mois suivant\"></a>";
        $html .= "\n</div>";
    ...
    <form action="assets/inc/process.inc.php" method="post">
        <fieldset>
            ...
            <input type="text" placeholder="YYYY-MM-DD HH:MM:SS" name="event_start"
                  id="event_start" value="$event->start" />
            <label for="event_end">End Time</label>
            <input type="text" placeholder="YYYY-MM-DD HH:MM:SS" name="event_end"
